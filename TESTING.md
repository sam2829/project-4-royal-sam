...
| Book a tee. | When user is logged in and clicks on any book a tee links, the user is taken to the first part of the booking process. User must fill out the book a tee form correctly before being able to continue to the next part. | Logged in to several different accounts and clicked on the book a tee links. Tried leaving fields blank in the book a tee form. | Every time the book a tee form loaded correctly and user was not allowed to continue to the second part unless both fields were filled out correctly. | Pass |
| Book a tee form correct date. | When user is selecting a date for the book a tee form, the user must select a date that is either the present date or future date. Any past date submitted will make a pop-up message appear asking the user to select a future date. | Loaded the book a tee form and tried submitting a past date. | User was unable to move on to the next part of the booking and pop-up message appears asking user to select a future date. | Pass |
...
| Booking date default. | When booking form first loads, the dates default is always the present date. | Loaded up the booking form on several different days. | The date always defaults to the present date. | Pass |
...
| Edit booking successful. | When user is trying to edit a booking and has gone through the process just like the original booking process. The user clicks update to submit, the user is then redirected back to the main my bookings page where they can find the booking they had edited and will now show the new information for the booking. Pop up message will also appear with information about the recent booking. | Went through the editing process for numerous bookings. | Every time I was redirected back to the main my bookings page where I could see the changes I had just made, and a pop-up message appeared showing me the recent booking information. | Pass |
...
| All forms cannot accept only spaces. | When a user is filling out any of the forms, the user is not able to just use only spaces in any of the required fields and submit the form. | In the leave a review form and comments form, for all required fields I attempted to submit the forms with only spaces. I did this all together and fields individually to prove if it worked. | Every time I tried to submit a form with any of the required fields only having spaces, the form was not submitted and the page was reloaded with an alert message, informing the user it's a required field and cannot contain white spaces alone. | Pass |
| Maximum character limit in forms. | When a user is filling out either the leave a review form or comments form, they should not be allowed to exceed the maximum characters in the field. In the leave a review form the title is 50 characters and content is 1000 characters. Comment for the body has a limit of 1000 characters. | Filled out both leave a review and comment forms several times, each time trying to exceed the character limit. | Every time I filled out the forms, the form itself would not let me go past the maximum characters in its field. | Pass |
| Special cases work in review title. | When user uses any special cases that aren't usually accepted in a URL, the form accepts it and creates a slug that is accepted to be used for the URL. | Created different titles for my review using "!" and other special cases not usually accepted. | Each time the slug was automatically created a name accepted to be used in the URL. | Pass |
| Unique review title. | When the user is filling out the leave a review form, the user must type in a unique title. If the title is not unique the form will be rendered again, with an alert message informing the user that the title they provided already exists. | Filled out the leave a review form several times and using titles I knew already existed. | The form would not be submitted and an alert message appeared informing me that the title already exists. | Pass |
...
| Admin delete photos from gallery. | As admin they are able to delete any existing photos from the site's gallery page. | Logged in as admin and tried deleting photos from the gallery. | I was successfully able to delete photos. | Pass |
| Error Page 404 works. | When user attempts something that creates an error 404, my custom 404.html page is rendered and user is able to click the link provided to go back to the homepage. | Created a 404 error. | My custom error 404 page was rendered and was able to click on link provided to navigate back to the homepage. | Pass |
| Error Page 403 works. | When user attempts something that creates an error 403, my custom 403.html page is rendered and user is able to click the link provided to go back to the homepage. | Created a 403 error. | My custom error 403 page was rendered and was able to click on link provided to navigate back to the homepage. | Pass |
| Error Page 403_csrf works. | When user attempts something that creates an error 403_csrf, my custom 403_csrf.html page is rendered and user is able to click the link provided to go back to the homepage. | Created a 403_csrf error. | My custom error 403_csrf page was rendered and was able to click on link provided to navigate back to the homepage. | Pass |
| Error Page 500 works. | When user attempts something that creates an error 500, my custom 404.html page is rendered and user is able to click the link provided to go back to the homepage. | Created a 500 error. | My custom error 500 page was rendered and was able to click on link provided to navigate back to the homepage. | Pass |
...
W3C CSS Validator was used to validate the site's CSS code.
...
| --- | --- | --- |
| Certain times weren't being removed from my bookings. | When a user was on my bookings page, certain times weren't being removed after the time had passed on that date. | After trying many things and then speaking to the tutor support, we thought it could be that the list of available times that didn't display four digits, for example they were "7:30", weren't being converted properly all the time so we changed the times to then read for example "07:30". This then seemed to have resolved the issue. |
| Special cases in review title. | When a user left a review and the title contained any special cases, for example a "!" used in "Amazing!". The reviews page would not be displayed due to the slug not being able to contain special cases. | Through researching this I found that using a slugify function instead, this would automatically clear any special cases and create a slug suitable for the URL. |
| Admin comments page hard to read. | When visiting the comments page as admin, I found it was displaying to much of the comment and post / review title which was making it difficult to read. | Through research I found that I was able to restrict the amount of the comment and title I wanted to be displayed. I did this using "get_short" function, which seemed to resolve the issue. |
| No maximum character limit on forms. | When user filled out the leave a review or comment forms, they were able to exceed the maximum characters set in the model. | Through research I found out that I can prevent the user from exceeding the maximum characters by setting the max length in the in the forms.py file. This then resolved the issue. |
| Only spaces in forms. | When user filled out the leave a review or comment forms, they were able to just use the space bar in the fields and submit. | Through research I found that the "is_valid()" function that's checking if the form is valid, checks for white spaces and if so, deems the form invalid. I then used this to then render form again and display a message informing the user that white spaces alone are not accepted. This then resolved the issue. |
| Access others edit and delete bookings. | I was able to copy the URL from one users edit / delete booking, then logout, paste in the URL and still be able access the edit and delete booking pages. | I then added login required to all classes that requires a user to be logged in, then also within the classes checked that the request user is the same as the booking user, if not this would render a 403 error and prevent users from accessing other users bookings. This resolved the issue. |
| Users trying to book same booking at the same time. | When two users are going through the booking process at the same time and try to book the same booking, the first user to complete booking was fine, but the other user has the page rendered again but doesn't know why and the times aren't now displayed correctly. | I found out that the "is_valid()" function was checking this automatically  for me, so I changed the code so when this happens and the form isn't valid as another user has just made the booking, the user who's booking wasn't valid now has the page rendered again with a message informing them that the booking could now have been taken and to select an available time. The times available are now displaying the correct times. |
