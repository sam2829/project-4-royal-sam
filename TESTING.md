...
| No maximum character limit on forms. | When user filled out the leave a review or comment forms, they were able to exceed the maximum characters set in the model. | Through research I found out that I can prevent the user from exceeding the maximum characters by adding  "len(data)" function in the forms.py file. This then resolved the issue. |
| Only spaces in forms. | When user filled out the leave a review or comment forms, they were able to just use the space bar in the fields and submit. | Through research I could raise a validation error to prevent this from being submitted by using a "data.strip()" function in the required fields. This then resolved the issue. |
